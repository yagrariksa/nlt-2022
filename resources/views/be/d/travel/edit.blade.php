...
<form
    ...
    <input type="text" value="{{ $peserta->nama }}" disabled>
    ...
    <input type="text" name="transportasi"
        value="{{ old('transportasi') ? old('transportasi') : $data->transportasi }}" placeholder="tipe transportasi">
    @if ($errors->has('transportasi'))
        <span class="error">{{ $errors->first('transportasi') }}</span>
    @endif
    <input type="datetime-local" name="datetime" id=""
        value="{{ old('datetime') ? old('datetime') : $data->tanggal . 'T' . ($data->jam ? $data->jam : '00:00') }}">
